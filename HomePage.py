...
st.markdown("""
<style>
.stApp { background-color: #F2E0C8 !important; color: #000000 !important; }
[data-testid="stAppViewContainer"] { background-color: #F2E0C8 !important; }
section[data-testid="stSidebar"] { background-color: #8B4513 !important; }
section[data-testid="stSidebar"] * { color: #FFFFFF !important; }
h1,h2,h3,h4 { color: #000000 !important; }
.card-container { display: flex; gap: 20px; margin-top: 20px; }
.card { flex: 1; padding: 25px; border-radius: 10px; text-align: center; color: white; font-weight: bold; }
.safe { background-color: #27AE60; } .warning { background-color: #F39C12; } .critical { background-color: #E74C3C; }
.card .count { font-size: 40px; } .card .label { font-size: 18px; }
...
try: bridges = pd.read_csv("data/bridges.csv")
except: st.error("bridges.csv file not found or empty."); st.stop()
...
<div class="card safe"><div class="count">{green_count}</div><div class="label">🟢 Safe Bridges</div></div>
<div class="card warning"><div class="count">{yellow_count}</div><div class="label">🟡 Warning Bridges</div></div>
<div class="card critical"><div class="count">{red_count}</div><div class="label">🔴 Critical Bridges</div></div>
</div>
""", unsafe_allow_html=True)

st.divider()

if red_count>0: st.markdown('<p class="blink">🔴 ALERT: Critical Flyover Requires Immediate Inspection</p>', unsafe_allow_html=True)
...
with left:
    ...
    for _,row in bridges.iterrows():
        folium.CircleMarker(
            ...
            fill_opacity=0.9,
            popup=f"<b>{row['name']}</b><br>Pincode: {row['pincode']}<br>Status: {row['status'].upper()}<br>Score: {row['score']}"
        ).add_to(m)
